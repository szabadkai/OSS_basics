...
function get_autotile_index(neighbor_mask) {
    ...
    switch(neighbor_mask) {
        case 0:   return 26;  // isolated - you specified this should be tile 26
        case 2:   return gm_autotile_map[0];  // N only -> tile 8
...
function place_autotile(grid_x, grid_y, layer_id) {
    ...
    // Create proper tile data
    // For GameMaker tilemaps, we need to create tile data with the correct index
    var tile_data = tile_index;
    
    // Place the tile using tilemap_set_at_pixel
    tilemap_set_at_pixel(tilemap_id, tile_data, grid_x * 32, grid_y * 32);
